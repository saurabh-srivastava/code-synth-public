...
{best_ub, best_ge}.  This subset is GENUINELY INVALID — it omits
...
private axiom kadane_sc2_invalid_witness :
    ∃ (n best i cur : Int) (A : Int → Int) (best' i' cur' : Int),
      ...
      (i' = (i + 1)) ∧
      ¬ (((∀ p q : Int, (((0 ≤ p) ∧ (p ≤ q) ∧ (q < i')) → ((sum_range A p (q + 1)) ≤ best')))) ∧ ((best' ≥ cur')))
...
theorem sc2_fallthrough_invalid :
    ∃ (n best i cur : Int) (A : Int → Int) (best' i' cur' : Int),
      ...
      (best' = (if (best ≥ (cur + (A i))) then best else (cur + (A i)))) ∧
      (i' = (i + 1)) ∧
      ¬ (((∀ p q : Int, (((0 ≤ p) ∧ (p ≤ q) ∧ (q < i')) → ((sum_range A p (q + 1)) ≤ best')))) ∧ ((best' ≥ cur'))) := by
