/-
kadane_max_subarray entry-bundle for τ subset C1, C3, C5, C6.
...
theorem sc0_fallthrough
    ...
    (h_init_cur : cur' = (A 0))
    (h_init_i : i' = 1) :
    ((1 ≤ i')) ∧ ((∀ p : Int, (((0 ≤ p) ∧ (p ≤ (i' - 1))) → ((sum_range A p i') ≤ cur')))) ∧ ((∀ p q : Int, (((0 ≤ p) ∧ (p ≤ q) ∧ (q < i')) → ((sum_range A p (q + 1)) ≤ best')))) ∧ ((best' ≥ cur')) := by
  subst_eqs
  refine ⟨?_, ?_, ?_, ?_⟩
  · omega
  · intro p hp
