/-
kadane_max_subarray entry-bundle for τ subset C2, C3, C4, C5.
...
theorem sc0_fallthrough
    ...
    (h_init_cur : cur' = (A 0))
    (h_init_i : i' = 1) :
    ((i' ≤ n)) ∧ ((∀ p : Int, (((0 ≤ p) ∧ (p ≤ (i' - 1))) → ((sum_range A p i') ≤ cur')))) ∧ ((∃ p : Int, ((0 ≤ p) ∧ (p ≤ (i' - 1)) ∧ ((sum_range A p i') = cur')))) ∧ ((∀ p q : Int, (((0 ≤ p) ∧ (p ≤ q) ∧ (q < i')) → ((sum_range A p (q + 1)) ≤ best')))) := by
  subst_eqs
  refine ⟨?_, ?_, ?_, ?_⟩
  · omega
  · intro p hp
