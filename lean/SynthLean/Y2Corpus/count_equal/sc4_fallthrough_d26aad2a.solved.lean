/-
count_equal's loop-inductive obligation, BRANCH 1 (A[i] ≠ t).
τ = {c = count_eq(A, i, t), 0 ≤ i, i ≤ n}.  Transition: i := i+1.

Argument: user_axiom_2 at k = i with (i ≥ 0 ∧ A i ≠ t) →
count_eq(A, i+1, t) = count_eq(A, i, t).
-/
...
theorem sc4_fallthrough
  ...
  refine ⟨?_, ?_, ?_⟩
  · have h_rec : count_eq A (i + 1) t = count_eq A i t :=
      user_axiom_2 A t i ⟨h_tau_1, h_branch⟩
