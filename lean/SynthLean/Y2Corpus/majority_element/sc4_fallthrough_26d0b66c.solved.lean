...
theorem sc4_fallthrough
    ...
    (h_tau_1 : (i ≤ n))
    (h_tau_2 : (cnt ≥ 0))
    (h_tau_3 : (∀ v : Int, ((cnt + (count_eq A i candidate)) ≥ (count_eq A i v))))
    (h_guard : ((i < n) ∧ (cnt > 0)))
    (h_trans_cnt : cnt' = (if ((A i) = candidate) then (cnt + 1) else (cnt - 1)))
    (h_trans_i : i' = (i + 1)) :
    ((0 ≤ i')) ∧ ((i' ≤ n)) ∧ ((cnt' ≥ 0)) ∧ ((∀ v : Int, ((cnt' + (count_eq A i' candidate)) ≥ (count_eq A i' v)))) := by
    ...
    · intro v
      ...
      exact bm_inv_preserve_b1 A n i candidate cnt h_pre h_tau_3 ⟨h_lt, h_cnt_pos⟩ v
