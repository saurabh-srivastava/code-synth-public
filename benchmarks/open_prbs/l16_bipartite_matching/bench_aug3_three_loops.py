"""bench_aug3_three_loops — C1.D K.3.2 full: 3-nested-loop AP search.

The full K.3.2: search over ALL three indices (u, v, w) for a
length-3 augmenting path u-v-z-w where z = M[v].  Uses nested
loops with `break` in the inner + `found == 0` outer-guard
short-circuit to exit all three loops on hit.
...
def _cite_middle_entry(chosen, hyp_for):
    return (
        "exact SynthLean.Y2Corpus.L16Aug3ThreeLoops.k32_3l_middle_entry "
...
def _cite_inner_entry(chosen, hyp_for):
    return (
        "exact SynthLean.Y2Corpus.L16Aug3ThreeLoops.k32_3l_inner_entry "
        "n_s0 found_s0 u_s0 v_s0 w_s0 M_s0 G_s0 "
        "n_s1 found_s1 u_s1 v_s1 w_s1 M_s1 G_s1 "
        + _pre_hyps() + " "
        "h_enc_tau_0 h_enc_tau_1 h_enc_tau_2 h_enc_tau_3 "
...
def _cite_inner_break(chosen, hyp_for):
    return (
        "exact SynthLean.Y2Corpus.L16Aug3ThreeLoops.k32_3l_inner_break "
        "n found u v w M G found' "
        + _pre_hyps() + " "
        "h_tau_0 h_tau_1 h_tau_2 h_tau_3 h_tau_4 h_tau_5 h_tau_6 "
...
def _cite_middle_body(chosen, hyp_for):
    return (
        "exact SynthLean.Y2Corpus.L16Aug3ThreeLoops.k32_3l_middle_body_ind "
        "n_s0 found_s0 u_s0 v_s0 w_s0 M_s0 G_s0 "
        "n_s1 found_s1 u_s1 v_s1 w_s1 M_s1 G_s1 "
        ...
        "h_i1_L2_tau_4 h_i1_L2_tau_5 h_i1_L2_tau_6 "
        "h_i1_L2_frame_n h_i1_L2_frame_u h_i1_L2_frame_v "
        "h_i1_L2_frame_M h_i1_L2_frame_G"
    )


def _cite_outer_body(chosen, hyp_for):
    return (
        "exact SynthLean.Y2Corpus.L16Aug3ThreeLoops.k32_3l_outer_body_ind "
        "n_s0 found_s0 u_s0 v_s0 w_s0 M_s0 G_s0 "
        "n_s1 found_s1 u_s1 v_s1 w_s1 M_s1 G_s1 "
        "n_s2 found_s2 u_s2 v_s2 w_s2 M_s2 G_s2 "
        + _pre_hyps() + " "
...
def _cite_final(chosen, hyp_for):
    # Flat-shape: (n, u, v, w, found, M, G) pre + (u', v', w', found') post.
    return (
        "exact SynthLean.Y2Corpus.L16Aug3ThreeLoops.k32_3l_final "
        "n u v w found M G u' v' w' found' "
        + _pre_hyps() + " "
...
_HELPER_REGISTRY = HelperRegistry(
    module_path="SynthLean.Y2Corpus.l16_aug3_three_loops.Helpers",
    entries=[
        # sc1: L1 entry-bundle.  Requires full τ_L0 (4 atoms).
        HelperEntry(
            helper_name="k32_3l_middle_entry",
        ...
        # sc2: L2 entry-bundle.  Requires full τ_L1 (6 atoms).
        HelperEntry(
            helper_name="k32_3l_inner_entry",
            applies_to=(lambda sc_kind, loop_id, branch_idx:
                        sc_kind == "safety-bundle-entry"
                        and loop_id == "L2"),
            required_atoms={
        ...
        # sc4: L2 inner break.  Requires full τ_L2 (7 atoms).
        HelperEntry(
            helper_name="k32_3l_inner_break",
            applies_to=(lambda sc_kind, loop_id, branch_idx:
                        sc_kind == "safety-bundle-post"
                        and loop_id == "L2"
                        and branch_idx == 0),
            required_atoms={
                "tau@L2": frozenset({0, 1, 2, 3, 4, 5, 6}),
            },
            cite=_cite_inner_break,
        ),
        # sc8: L1 chain-bundle-post (middle body inductive).
        HelperEntry(
            helper_name="k32_3l_middle_body_ind",
            applies_to=(lambda sc_kind, loop_id, branch_idx:
                        sc_kind == "safety-bundle-post"
                        and loop_id == "L2"
        ...
        HelperEntry(
            helper_name="k32_3l_outer_body_ind",
            applies_to=(lambda sc_kind, loop_id, branch_idx:
                        sc_kind == "safety-bundle-post"
                        and loop_id == "L1"
                        and branch_idx is None),
...
PROBLEM = Problem(
    ...
    atoms = {
        ...
        # Inner branch 0: AP found.  Set found, break.  Note:
        # also require M[u] == -1 (u unmatched) to ensure the
        # path is a real AP — not just any 4 distinct edge-
        # connected vertices.
        "g@B3.0": [(
    ...
    dump_lean_failures_dir = (
        "lean/SynthLean/Y2Corpus/l16_aug3_three_loops"
    ),
