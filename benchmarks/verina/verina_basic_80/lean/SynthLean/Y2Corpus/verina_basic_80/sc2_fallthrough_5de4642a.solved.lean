/-
verina_basic_80 (only_once) — loop-inductive, BRANCH 0
(A[i] == key).  Transition: c := c+1, i := i+1.
Argument: user_axiom_1 at k = i with (i ≥ 0 ∧ A i = key) →
count_occ(A,key,i+1) = count_occ(A,key,i) + 1.
-/
...
theorem sc2_fallthrough
  ...
  refine ⟨?_, ?_, ?_, ?_⟩
  · have h_rec : count_occ A key (i + 1) = count_occ A key i + 1 :=
      user_axiom_1 A key i ⟨h_tau_1, h_branch⟩
